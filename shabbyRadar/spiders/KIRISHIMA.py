...
class JdSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for num in range(0, 1825):
            ...
            url = 'http://data.chinafund.cn/kf/' + output_yesterday + '.html'
    ...
    def parse(self, response):
        ...
        if response.url != response.meta['originURL']:
            url = 'http://data.chinafund.cn/kf/' + targetDate.replace("-", "") + '.html'
            ...
            for tr in soup.findAll("tr"):
                ...
                item['id'] = tr.findAll("td")[2].string
                item['name'] = tr.findAll("td")[3].string
                item['value1'] = tr.findAll("td")[5].string
                item['value2'] = tr.findAll("td")[6].string
                item['delta'] = tr.findAll("td")[7].string
                item['deltaPCT'] = tr.findAll("td")[8].string
                yield item
    ...
    def get_filter(self):
        with open('D:\\NMCC\\myNavy\\AegisCombatSystem\\KIRISHIMA\\filter.txt', 'r') as f:
    ...
    def insert_by_many(self):
        ...
        try:
            sql = "INSERT INTO fund_open VALUES (%s, %s, %s, %s, %s, %s, %s)"
            # 批量插入
